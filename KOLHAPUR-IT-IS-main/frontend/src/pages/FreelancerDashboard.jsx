...
import { statsAPI, projectsAPI, intelAPI, assessmentAPI } from '../services/api';
...
const FreelancerDashboard = () => {
  ...
  useEffect(() => {
    ...
    setAssignmentsError("");
    assessmentAPI.getPendingVmAssessments()
      .then((res) => setAssignments(res.data?.assignments || []))
      .catch(() => {
        setAssignments([]);
        setAssignmentsError("Assessment assignments could not be loaded right now.");
      })
      ...
      <div className="max-w-[1400px] mx-auto space-y-8">
        ...
        {activeProject ? (
          ...
          <div className="bg-white rounded-[2rem] p-12 border-2 border-dashed border-slate-200 flex flex-col items-center text-center space-y-4">
            ...
            <button onClick={() => navigate('/jobs')} className="px-8 py-3 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 transition shadow-lg shadow-blue-200">
              Discover Jobs
            </button>
          </div>
        )}
        ...
        {assignmentsLoading ? (
          <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
            <p className="text-sm font-semibold text-slate-500">Loading assessment assignments...</p>
          </div>
        ) : assignmentsError ? (
          <div className="rounded-[2rem] border border-rose-200 bg-rose-50 p-6 shadow-sm">
            <p className="text-sm font-semibold text-rose-700">{assignmentsError}</p>
          </div>
        ) : assignments.length ? (
          <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
              <div>
                <p className="text-xs font-black uppercase tracking-[0.22em] text-emerald-600">Coding Assessments</p>
                <h2 className="mt-2 text-2xl font-black text-slate-900">Tests attached to your applications</h2>
              </div>
              <p className="text-sm text-slate-500">Server timer, snapshots, and violation tracking stay attached to the submission.</p>
            </div>
            <div className="mt-5 grid gap-4 lg:grid-cols-2">
              {assignments.map((assignment) => (
                <div key={`${assignment.jobId}-${assignment.test?._id}`} className="rounded-[24px] border border-slate-200 bg-slate-50 p-5">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="text-lg font-black text-slate-900">{assignment.jobTitle}</p>
                      <p className="mt-1 text-sm text-slate-500">{assignment.test?.title} · {assignment.test?.language?.toUpperCase()} · {assignment.test?.durationMinutes} min</p>
                    </div>
                    <span className={`rounded-full px-3 py-1 text-xs font-bold ${assignment.submitted ? 'bg-emerald-100 text-emerald-700' : assignment.sessionStatus === 'active' ? 'bg-blue-100 text-blue-700' : 'bg-amber-100 text-amber-700'}`}>
                      {assignment.submitted ? 'Submitted' : assignment.sessionStatus === 'active' ? 'In Progress' : 'Available'}
                    </span>
                  </div>
                  <div className="mt-4 flex items-center justify-between gap-3">
                    <p className="text-sm text-slate-500">{assignment.matchScore ? `Match score ${assignment.matchScore}%` : 'Assessment available on this application'}</p>
                    <button
                      onClick={() => navigate(`/test/${assignment.jobId}`)}
                      className="rounded-xl bg-slate-900 px-4 py-2 text-sm font-bold text-white hover:bg-slate-800"
                    >
                      {assignment.submitted ? 'View Status' : assignment.sessionStatus === 'active' ? 'Resume Test' : 'Start Test'}
                    </button>
                  </div>
                </div>
              ))}
