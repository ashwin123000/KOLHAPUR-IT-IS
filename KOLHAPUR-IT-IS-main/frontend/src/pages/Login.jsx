...
export default function Login({ role, onLogin }) {
  ...
  const processLoginData = (data) => {
    ...
    if (!tokenValue) {
      setError('Authentication failed: token not returned.');
      return false;
    }
    const actualRole = data.role || role;

    // Block cross-portal login
    if (data.role && actualRole !== role) {
      setError(
        role === 'client'
    ...
    if (data.user?.name) localStorage.setItem('full_name', data.user.name);

    if (typeof onLogin === 'function' && data.user) {
      onLogin(data.user);
    }

    const displayName = data.user?.name || email.split('@')[0];
    setWelcomeName(displayName);
    if (data.user && data.user.registration_complete === false) {
  ...
  const handlePasswordLogin = async (e) => {
    ...
    try {
      ...
      if (data.access_token || data.token) processLoginData(data);
    } catch (err) {
  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || err.response?.data?.detail || 'Invalid or expired OTP.');
    } finally {
      setLoading(false);
    }
  };
